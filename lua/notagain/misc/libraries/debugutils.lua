...
do
	...
	function META:__index(key)
		return rawget(debugutils.SliderVars, key) or 0
	end
	...
	if SERVER then
		function debugutils.ReceiveSliderVar(ply, _, args)
			if true or ply:IsAdmin() then
				...
				rawset(debugutils.SliderVars, key, tonumber(value))
			end
		end
...
if CLIENT then
	...
	function debugutils.GarbageCollect()
		for id, data in pairs(debugutils.PrintQueue) do
			if data.time < CurTime() then
				debugutils.PrintQueue[id] = nil
			end
		end
	end
	...
	function debugutils.Print(id, pos, ...)
		...
		local str = debugutils.ArgsToString(...)
		debugutils.PrintQueue[id] = {
	...
	function debugutils.HUDPaint()
		for _, data in pairs(debugutils.PrintQueue) do
			local pos
	...
	function debugutils.ReceiveServerMessage(umr)
		...
		debugutils.Print(id, pos, unpack(args))
	end
